...
export const AlertProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <AlertContext.Provider value={{ showAlert }}>
      {alert && (
        <div
          className={`fixed z-50 top-[10svh] flex items-center justify-center px-2 py-1 rounded-lg text-center w-fit ${isVisible ? "opacity-100" : "opacity-0"} transition-all duration-500 ease-in-out ${getAlertClasses(alert.type)}`}
        >
          {alert.message}
        </div>
      )}
